...
**Skyrim Special Edition / Anniversary Edition only.** Legendary Edition (32-bit, BSA v103, zlib compression, old HKX format) is explicitly out of scope and will never be supported. All format assumptions — BSA v104 (SE) / v105 (AE, same structure — Bethesda only bumped the version number), LZ4 block compression, 64-bit form IDs, SE-era HKX — are made for SE/AE. Do not add LE fallback paths.
...
| NIF I/O | **nifly** (ousnius/nifly) | GPL-3.0; read/write NIF SE; used by BodySlide/Outfit Studio; see §5.2 |
| TRI parsing | Custom thin parser | FRTRI003 format; read-only; ~200 lines; see §5.3 |
| Plugin I/O | **Mutagen** (via SctBridge) | MIT-licensed; read/write ESP/ESM/ESL; see §7 |
| Language | C++23 | MSVC, Windows only for now |
| Runtime foundation | **Engine Relay** (SKSE plugin) | Custom Havok user state (character controller + physics disabled); dynamic behavior injection via reference relay; PlayerCamera subclass with frame-by-frame transform API. See §10.9 |
| Runtime coordinator | **True Cinematics** (SKSE plugin) | Cross-actor sync via annotation event routing; scene lifecycle (spawn, cleanup); annotation dispatch (music, environment). See §10 |
...
## 5. The HKX / NIF Dual-Skeleton Architecture
...
### 5.1 BSBehaviorGraphExtraData — The Invisible Import Bridge
...
### 5.2 NIF Binary Format & Version Fingerprinting
...
### 5.3 Mesh Skinning — BSSkin::Instance
...
**Consequence for SCT**: nifly gives us `boneRefs` names + `BSSkinBoneData` inverse bind matrices. We already have live bone world transforms from the Havok pose system (§4). The skinning math is a standard linear blend skinning sum — no skeleton NIF required at runtime.

---

## 5.4 FaceGen & Expression Morph Pipeline
...
4. Terrain heightfield (see §8)

---

## 7. Plugin Integration — Mutagen
...
## 8. Terrain & Scene Environment

### 8.1 LAND Record Heightfield
...
## 9. Animated Props, Lights & Cameras
...
As **rendered objects**: requires the GPU render pipeline (§6). In the interim, lights render as icons/glyphs in the viewport with a radius indicator circle.
...
## 10. Scene Graph & Runtime Export Model

### 10.1 The Core Mapping
...
### 10.3 Scene Node Graph → Behavior HKX (Per Actor)
...
### 10.5 Per-Actor Behavior Injection
...
### 10.6 Multi-Actor Synchronization
...
### 10.9 YAML Sidecar — Metadata Only
...
- The status bar carries four zones (§14.8).
...
- The right Inspector panel replaces standard object properties with the Morph Sliders layout (§14.7.4).
...
#### 14.7.1 Nothing Selected — Scene Properties
...
#### 14.7.3 Clip Selected (in timeline)
...
#### 14.7.4 Face Mode — ARKit Morph Sliders
...
#### 14.7.5 Bone Selected (Anim Editor)
...
### 14.12 Anim Editor — Dopesheet and Curve Editor
...
### 14.13 Missing Components — Priority Order
...
### 14.14 ARKit Naming Contract and Import Validation
...
### 14.15 Phase-by-Phase UI Additions
