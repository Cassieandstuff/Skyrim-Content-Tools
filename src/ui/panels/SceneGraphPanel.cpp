...
void SceneGraphPanel::Draw() {
    ...
    // Draw three placeholder node boxes to suggest the graph layout
    auto DrawNode = [&](float x, float y, const char* label, ImU32 col) {
        ImVec2 np = { pos.x + x, pos.y + y };
        ImVec2 ne = { np.x + 110.0f, np.y + 36.0f };
        dl->AddRectFilled(np, ne, IM_COL32(28, 34, 46, 255), 4.0f);
        dl->AddRect(np, ne, col, 4.0f);
        ImVec2 ts = ImGui::CalcTextSize(label);
        dl->AddText({ np.x + (110.0f - ts.x) * 0.5f, np.y + 10.0f },
                    IM_COL32(200, 210, 230, 200), label);
    };

    const ImU32 kWire = IM_COL32(50, 90, 160, 180);
    if (size.x > 140 && size.y > 160) {
        const float cx = size.x * 0.5f - 55.0f;
